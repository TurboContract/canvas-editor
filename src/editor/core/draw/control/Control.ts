...
export class Control {
    ...
    public renderHighlightList(ctx: CanvasRenderingContext2D, pageNo: number) {
        const highlightMatchResult =
            this.controlSearch.getHighlightMatchResult()
    ...
    // 是否元素包含完整控件元素
    public getIsElementListContainFullControl(
    ...
    public addPlaceholder(startIndex: number, context: IControlContext = {}) {
        ...
        // 优先使用默认控件样式
        const anchorElementStyleAttr = pickObject(
            startElement,
            CONTROL_STYLE_ATTR,
        )
                ...
                if (type === ControlType.TEXT || type === ControlType.DATE) {
                    ...
                    const innerText = code
                        ...
                        .map(
                            (selectCode) =>
                                valueSets?.find(
                                    (valueSet) => valueSet.code === selectCode,
                                )?.value,
                        )
                        .filter(Boolean)
                        .join('')
    ...
    public setPropertiesById(payload: ISetControlProperties) {
        ...
        for (const key in pageComponentData) {
            ...
            const elementList = zipElementList(
                pageComponentData[pageComponentKey]!,
            )
