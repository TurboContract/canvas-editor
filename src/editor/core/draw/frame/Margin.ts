...
export class Margin {
    ...
    public render(ctx: CanvasRenderingContext2D, pageNo: number) {
        ...
        const rightTopPoint: [number, number] = [
            width - margins[1],
            margins[0],
        ];
        const leftBottomPoint: [number, number] = [
            margins[3],
            height - margins[2],
        ];
        ...
        // 下左
        ctx.moveTo(
            leftBottomPoint[0] - marginIndicatorSize,
            leftBottomPoint[1],
        );
        ctx.lineTo(...leftBottomPoint);
        ctx.lineTo(
            leftBottomPoint[0],
            leftBottomPoint[1] + marginIndicatorSize,
        );
        // 下右
        ctx.moveTo(
        ...
        ctx.lineTo(
            rightBottomPoint[0],
            rightBottomPoint[1] + marginIndicatorSize,
        );
        ctx.stroke();
        ctx.restore();
    }
