...
export class TableTool {
    ...
    private tableFormulaYes: HTMLButtonElement | null;
    private tableFormulaNo: HTMLButtonElement | null;
    ...
    constructor(draw: Draw) {
        ...
        // x、y轴
        this.tableFormulaYes = null;
        this.tableFormulaNo = null;
    ...
    public dispose() {
        this.tableFormulaYes?.remove();
        this.tableFormulaNo?.remove();
    ...
    public render() {
        ...
        const { defaultTrMinHeight } = table;
        const elementList = this.draw.getOriginalElementList();
        const positionList = this.position.getOriginalPositionList();
        const element = elementList[index!];
        const isTableFormulaEditing =
        ...
        for (let r = 0; r < trList!.length; r++) {
            ...
            for (let d = 0; d < tr.tdList.length; d++) {
                ...
                rowBorder.classList.add(
                    `${EDITOR_PREFIX}-table-tool__border__row`,
                );
                ...
                colBorder.classList.add(
                    `${EDITOR_PREFIX}-table-tool__border__col`,
                );
        ...
        if (isTableFormulaEditing) {
            const yesButton = document.createElement('button');
            yesButton.innerText = 'да';
            yesButton.classList.add(`${EDITOR_PREFIX}-table-tool_formula_yes`);
            yesButton.style.zIndex = '9999';
            yesButton.style.position = 'absolute';
            yesButton.style.width = `${(80 * scale) / 2}px`;
            yesButton.style.height = `${(80 * scale) / 2}px`;
            yesButton.style.top = `${tableY! * scale}px`;
            yesButton.style.left = `${tableWidth! + tableX! + 20 * scale}px`;
            yesButton.onclick = (evt) => {
                this.draw.getOptions().isTableFormulaEditing = false;
                this.draw.calculateFormula(
            ...
            this.container.append(yesButton);
            this.tableFormulaYes = yesButton;

            const noButton = document.createElement('button');
            noButton.innerText = 'нет';
            noButton.classList.add(`${EDITOR_PREFIX}-table-tool_formula_no`);
            noButton.style.zIndex = '9999';
            noButton.style.position = 'absolute';
            noButton.style.width = `${(80 * scale) / 2}px`;
            noButton.style.height = `${(80 * scale) / 2}px`;
            noButton.style.top = `${tableY + defaultTrMinHeight * scale}px`;
            noButton.style.left = `${tableWidth! + tableX! + 20 * scale}px`;
            noButton.onclick = (evt) => {
                this.draw.getOptions().isTableFormulaEditing = false;
                this.draw.calculateFormula(
                    elementList[index].trList[trIndex].tdList[tdIndex].value,
                    index,
            ...
            this.container.append(noButton);
            this.tableFormulaNo = noButton;
        }
    ...
    private _mousedown(payload: IAnchorMouseDown) {
        ...
        if (order === TableOrder.ROW) {
            anchorLine.classList.add(
                `${EDITOR_PREFIX}-table-anchor__line__row`,
            );
            ...
            anchorLine.classList.add(
                `${EDITOR_PREFIX}-table-anchor__line__col`,
            );
    ...
    private recalculateFormula(element: IElement, index: number) {
        element.trList.forEach((row, rowIndex) => {
