...
export class ContextMenu {
    ...
    private _proxyContextMenuEvent = (evt: MouseEvent) => {
        ...
        const isRegisterContextMenu = renderList.some(
            (menu) => !menu.isDivider,
        );
    ...
    private _render(payload: IRenderPayload): HTMLDivElement {
        ...
        const rect = editorBody
            ? editorBody.getBoundingClientRect()
            : { top: 0 };
        const topPosition = rect.top + window.scrollY;

        const menuContainer = document.querySelector('.editor__menu');
        const rectMenu = menuContainer
            ? menuContainer.getBoundingClientRect()
            : { height: 0 };
        const menuHeight = rectMenu.height;
        ...
        // 下侧空间不足时，以菜单底部作为起始点
        const innerHeight = window.innerHeight - 106;
        const contextMenuHeight = contextmenuRect.height;
        let adjustTop =
            top + contextMenuHeight > innerHeight
                ? top - contextMenuHeight
                : top;
        if (adjustTop < topPosition + menuHeight) {
            adjustTop = topPosition + menuHeight;
        }
