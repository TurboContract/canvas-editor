...
export class CommandAdapt {
    ...
    public setPositionContext(range: IRange) {
        ...
        if (tableId) {
            const tableElementIndex = elementList.findIndex(
                (el) => el.id === tableId,
            )
    ...
    public rowFlex(payload: RowFlex) {
        ...
        // В таблицах режимы "по ширине/распределить" нестабильны, т.к.
        // ширина строки привязана к контентной области ячейки.
        // Безопасно нормализуем их к левому выравниванию.
        const nextRowFlex =
            isTable &&
            (payload === RowFlex.ALIGNMENT || payload === RowFlex.JUSTIFY)
                ? RowFlex.LEFT
                : payload
        if (
            ...
            const tableElement =
                this.draw.getOriginalElementList()[positionContext.index]
            const td =
                tableElement?.trList?.[positionContext.trIndex]?.tdList?.[
            ...
            if (
                ...
                td.rowFlex = nextRowFlex
                td.value.forEach((element) => {
                    element.rowFlex = nextRowFlex
                })
                const isSetCursor = startIndex === endIndex
                const curIndex = isSetCursor ? endIndex : startIndex
                this.draw.render({ curIndex, isSetCursor })
    ...
    public tableTdTextOrientation(payload: TextOrientation) {
        const isReadonly = this.draw.isReadonly()
        if (isReadonly) return
        const rowCol = this.draw.getTableParticle().getRangeRowCol()
        if (!rowCol) return
        for (let r = 0; r < rowCol.length; r++) {
            ...
            for (let c = 0; c < row.length; c++) {
                const td = row[c]
                if (!td || td.textOrientation === payload) {
                    continue
                }
                td.textOrientation = payload
                if (payload === TextOrientation.HORIZONTAL) {
                    delete td.rowFlex
                } else if (td.value.length) {
                    if (td.rowFlex === undefined) {
                        td.rowFlex =
                            td.value.find((e) => e.rowFlex)?.rowFlex ??
                            RowFlex.LEFT
                    }
                    const flex = td.rowFlex
                    td.value.forEach((e) => {
                        e.rowFlex = flex
                    })
                }
    ...
    public tableTdSlashType(payload: TdSlash) {
        ...
        tdList.forEach((td) => {
            ...
            const slashTypeIndex = td.slashTypes.findIndex(
                (type) => type === payload,
            )
            if (isSetTdSlashType) {
                if (!~slashTypeIndex) {
    ...
    public getHTML(): IEditorHTML {
        ...
        return {
            header: createDomFromElementList(headerElementList, options)
                .innerHTML,
            main: createDomFromElementList(mainElementList, options).innerHTML,
            footer: createDomFromElementList(footerElementList, options)
                .innerHTML,
        }
    }
    ...
    public getRangeContext(): RangeContext | null {
        ...
        if (!~startIndex && !~endIndex) return null
        const positionList = this.position.getPositionList()
        const hasStartPosition = !!positionList[startIndex]
        const hasEndPosition = !!positionList[endIndex]
    ...
    public getRangeParagraph(): IElement[] | null {
        ...
        return paragraphElementList
            ? zipElementList(paragraphElementList)
            : null
    }
    ...
    public locationControl(
        ...
        for (const context of data) {
            ...
            if (locationContext) {
                ...
                this.position.setPositionContext(
                    locationContext.positionContext,
                )
    ...
    public getParagraphId(): object | null {
        ...
        const coordinates =
            positionList[
                elementList.findIndex((elem) => elem?.id === element?.id)
            ].coordinate
        let isList = false
        if (element?.listId) {
            isList = true
        ...
        return element && element.id
            ? {
                  ...
                  isList,
                  pageNo,
                  height: this.draw.getHeight() + 23 - pageNo,
              }
    ...
    public setPlaceholderText() {
        this.draw.setPlacholderText()
    }

    public setDisabledText() {
        this.draw.setDisabledText()
    }

    public transformCellIntoFormula() {
        this.draw.transformCellIntoFormula()
    }

    public deleteCellFormula() {
        this.draw.deleteFormula()
    }

    public editCellFormula() {
        this.draw.editFormula()
    }
}
