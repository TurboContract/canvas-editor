...
export class Command {
    ...
    public executeTableTdTextOrientation: CommandAdapt['tableTdTextOrientation']
    ...
    public executePlaceHolderText: CommandAdapt['setPlaceholderText']
    public executeDisabledText: CommandAdapt['setDisabledText']
    public executeTransformCellIntoFormula: CommandAdapt['transformCellIntoFormula']
    public executeDeleteCellFormula: CommandAdapt['deleteCellFormula']
    public executeEditCellFormula: CommandAdapt['editCellFormula']
    ...
    constructor(adapt: CommandAdapt) {
        ...
        this.executeInsertTableBottomRow =
        ...
        this.executeCancelMergeTableCell =
            adapt.cancelMergeTableCell.bind(adapt)
        this.executeTableTdVerticalAlign =
            adapt.tableTdVerticalAlign.bind(adapt)
        this.executeTableTdTextOrientation =
            adapt.tableTdTextOrientation.bind(adapt)
        ...
        this.executeSetControlProperties =
        ...
        this.executePlaceHolderText = adapt.setPlaceholderText.bind(adapt)
        this.executeDisabledText = adapt.setDisabledText.bind(adapt)
        this.executeTransformCellIntoFormula =
            adapt.transformCellIntoFormula.bind(adapt)
        this.executeDeleteCellFormula = adapt.deleteCellFormula.bind(adapt)
        this.executeEditCellFormula = adapt.editCellFormula.bind(adapt)

        // Debug: log all execute* calls with payload/args.
        // This helps to observe what is being executed from UI integrations.
        Object.keys(this).forEach((key) => {
            if (!key.startsWith('execute')) return
            const original = (this as any)[key]
            if (typeof original !== 'function') return;
            (this as any)[key] = (...args: unknown[]) => {
                // eslint-disable-next-line no-console
                console.debug(`[canvas-editor] ${key}`, ...args)
